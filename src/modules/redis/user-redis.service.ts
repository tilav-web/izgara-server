...
export class UserRedisService {
  ...
  async setRoleSocketClientId({
    ...
    const key = `role_sockets:${role}`;
    await this.redis.sadd(key, client_id);
    await this.redis.expire(key, ttl);
  }

  async removeRoleSocketClientId(role: AuthRoleEnum, client_id: string) {
    const key = `role_sockets:${role}`;
    await this.redis.srem(key, client_id);
  }

  async getRoleSocketClients(role: AuthRoleEnum): Promise<string[]> {
    const key = `role_sockets:${role}`;
    return await this.redis.smembers(key);
  }
}
