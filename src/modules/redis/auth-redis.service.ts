...
export class AuthRedisService {
  ...
  async setNotFoundAuth({
    phone,
    id,
    ttl = CACHE_TTL.AUTH_NOT_FOUND,
  }: {
    phone?: string;
    id?: number;
    ttl?: number;
    ...
    if (!phone && !id)
      throw new Error(
        'Set not found redis funksiyasiga id yoki phone kelishi kerak!',
      );
    if (phone) {
