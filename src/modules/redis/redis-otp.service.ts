...
export class RedisOtpService {
    ...
    async setOtpByPhone({ phone, code, ttl = CACHE_TTL.OTP_TTL }: { phone: string, code: number, ttl?: number }): Promise<void> {
    ...
    async verifyOtpByPhone({ phone, code }: { phone: string, code: number }): Promise<0 | 1 | 2> {
        const key = `otp:phone:${phone}`;
