...
export class SeedService {
  ...
  constructor(
    ...
    private readonly coinSettingsRepository: Repository<CoinSettings>,
  ...
  async createDeafultCoinService() {
    try {
      ...
      const max_coins_per_order =
        parseFloat(
          this.configService.get<string>('MAX_COINS_PER_ORDER') ?? '0',
        ) ?? undefined;
      const min_spend_limit =
        parseFloat(this.configService.get<string>('MIN_SPEND_LIMIT') ?? '0') ??
        undefined;
      const spend_amount_for_one_coin =
        parseFloat(
          this.configService.get<string>('SPEND_AMOUNT_FOR_ONE_COIN') ?? '0',
        ) ?? undefined;
      const value_per_coin =
        parseFloat(this.configService.get<string>('VALUE_PER_COIN') ?? '0') ??
        undefined;
      await this.coinSettingsRepository.save({
        value_per_coin,
